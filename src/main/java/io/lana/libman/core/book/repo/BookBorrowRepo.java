...
public interface BookBorrowRepo extends PagingAndSortingRepository<BookBorrow, String> {
    ...
    @Query(value = "select b from BookBorrow b join fetch b.book bo join fetch bo.info where b.reader.id = :id and b.returned is false",
    ...
    @Query(value = "select b from BookBorrow b join fetch b.book bo join fetch bo.info i " +
            "where b.reader.id = :id and b.returned is false and lower(i.title) like lower(:query)",
            countQuery = "select count(b.id) from BookBorrow b join b.book bo join bo.info i " +
                    "where b.reader.id = :id and b.returned is false and lower(i.title) like lower(:query) ")
